...
and we have a box hooked up that reads mileage numbers. We've got a box glued to his dash that lights up 
...
const palindrome = (input) => {
	...
	if (temp == tempInput) return 1;
	return 0;
};

const decrementing = (input) => {
	let temp = 0,
		tempInput = input;
	(x = 10), (temp2 = 0);
	while (input > 6) {
		temp = input % x;
		temp2 = console.log(temp, temp2);
		x *= 10;
	}
};

function isInteresting(number, awesomePhrases) {
	let tempNumb = number.toString();
	console.log(tempNumb);
	if (!tempNumb < 90) {
		for (let i = 0; i, tempNumb.length; i++) {}
	}
}

// isInteresting(32678, [ 1337, 256 ]);
console.log(decrementing(987));
